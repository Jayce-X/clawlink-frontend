...
export default function AgentBookModal({ open, onClose, onTryAgent, initialQuery }: Props) {
  ...
  return (
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[720px] max-h-[80vh] flex flex-col overflow-hidden mx-4">
        {/* Header */}
        <div className="flex items-center justify-between px-7 pt-6 pb-3">
          <h2 className="text-xl font-bold text-zinc-900">Select expert agents and skills</h2>
          <button
            onClick={onClose}
        ...
        {/* Search pill showing current query */}
        {initialQuery && (
          <div className="px-7 pb-3">
            <div className="flex items-center gap-2 rounded-full bg-zinc-100 px-4 py-2 max-w-full">
              <Search className="h-3.5 w-3.5 text-zinc-400 flex-shrink-0" />
              <span className="text-sm text-zinc-500 truncate">{initialQuery}</span>
            </div>
          </div>
        )}
        ...
        {/* Search bar */}
        <div className="px-7 pb-3">
          <div className="flex items-center gap-2 rounded-full bg-zinc-50 border border-zinc-200 px-4 py-2.5">
            ...
            <button
              ...
              className="px-4 py-1 rounded-full bg-zinc-900 text-white text-xs font-semibold hover:bg-zinc-800 transition-colors"
            >
              Search
            </button>
          </div>
        ...
        <div className="px-7 pb-3">
          <div className="flex gap-2 flex-wrap">
            {["Social Media", "Marketing", "Stock", "Legal", "Translation", "Code", "Data"].map((tag) => (
              <button
                key={tag}
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {results.map((result) => (
                <div
                  key={result.agent_id}
                  className="rounded-2xl bg-zinc-50 border border-zinc-200/80 p-5 hover:shadow-sm transition-shadow"
                >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {/* Try button */}
                    <button
                      onClick={() => onTryAgent(result.agent_id)}
